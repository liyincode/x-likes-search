...
- **图片画廊**：响应式浏览、大图预览、键盘切换、单张或批量下载图片，并可返回 X 查看原推文。
- **安全同步**：添加最新点赞；只有在确认到达时间线末尾后，才会移除已经取消点赞的记录。
- **本地优先**：没有服务器、遥测或数据上传。
- **CSV 导出**：用 Excel 或 Numbers 打开当前结果；同时为高级用途保留原始 JSON 导出。
...
- 点击图片可以打开大图预览，并通过方向键或界面按钮在完整结果集中切换；点击预览中的下载按钮，可保存当前原始尺寸图片。
- 点击 **select** 选择图片，再点击 **download**，即可把原始尺寸图片保存到 `Downloads/x-likes-search/`。
...
| `downloads` | 只保存你在大图预览或画廊选择中明确下载的原始尺寸图片。 |
